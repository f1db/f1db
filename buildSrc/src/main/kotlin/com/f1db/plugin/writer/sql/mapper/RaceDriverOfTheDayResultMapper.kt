...
/**
 * The driver of the day mapper.
 *
 * @author Marcel Overdijk
 */
@Mapper(unmappedTargetPolicy = ReportingPolicy.ERROR)
interface RaceDriverOfTheDayResultMapper : RecordUnmapper<DriverOfTheDayResult, RaceDataRecord> {
    ...
    @Mapping(target = "driverOfTheDayPercentage", source = "percentage")
    override fun unmap(source: DriverOfTheDayResult): RaceDataRecord

    fun unmap(source: DriverOfTheDayResult, type: RaceDataType): RaceDataRecord {
        return unmap(source).apply { setType(type.name) }
    }

    fun unmap(sources: List<DriverOfTheDayResult>, type: RaceDataType): List<RaceDataRecord> {
